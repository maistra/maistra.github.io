...
In order to run the container, the container image needs to build it with the command:
````
$ docker-build-image:
````
This is only required if the image is not available on your local computer.
...
The goal "docker-run" is used for internal purpose to avoid creating a script file to run tests and Hugo in the container. The container image is designed for testing purposes and not for productive use cases.
