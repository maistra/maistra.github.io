...
[IMPORTANT]
====
You can only use the 3scale Istio adapter with {ProductName} versions 2.0 and below. The Mixer component was deprecated in release 2.0 and removed in release 2.1. For {ProductName} versions 2.1.0 and later you should use the xref:/docs/ossm-threescale-webassembly-module.adoc[3scale WebAssembly module].

If you want to enable 3scale backend cache with the 3scale Istio adapter, you must also enable Mixer policy and Mixer telemetry. See xref:/docs/ossm-create-smcp.adoc#ossm-create-smcp[Deploying the Red Hat OpenShift Service Mesh control plane].
====
