...
For information about mTLS, see xref:/docs/ossm-security.adoc#ossm-security-mtls_ossm-security[Enabling mutual Transport Layer Security (mTLS)]
...
Istiod manages client certificates and private keys used by service proxies.  By default, Istiod uses a self-signed certificate for signing, but you can configure a custom certificate and private key. For more information about how to configure signing keys, see xref:/docs/ossm-security.html#ossm-cert-manage_ossm-security[Adding an external certificate authority key and certificate]
...
For an example, of how to configure Pilot resource scheduling see xref:/docs/ossm-custom-resources.adoc#ossm-cr-pilot_ossm-custom-resources-v2x[Istio Pilot configuration]
