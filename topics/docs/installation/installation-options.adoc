...
|`version`
|Maistra version that should be deployed in this control plane installation. Maistra operator supports installing different versions of a control plane. This means that you can have multiple service meshes in your cluster with different Maistra versions. If this field is not present, Maistra operator will install the latest version it supports. For Maistra operator `1.1` one of the following values are allowed in this field: [`v1.0`, `v1.1`].
