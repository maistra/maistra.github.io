...
If you installed the Maistra operator using the Automatic Approval Strategy, then
the operator and control plane will update automatically. If not, then you will
have to manually upgrade each of the operators. In either case, the application
sidecars must be updated separately from the control plane.

If your deployment is using using automatic injection, then the application only
needs to be restarted in order to update the pod. Execute the following to
