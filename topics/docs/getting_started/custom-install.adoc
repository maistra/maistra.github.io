...
|=======
|Parameter |Description |Default
|enabled | The parameter controls whether Istio CNI should be used for side-proxy network configuration. Only has effect when multitenancy has been enabled. | false
|=======
...
|Parameter |Description |Default
|hub
|The hub that the Operator uses to pull Jaeger images. Any valid image repo
|`jaegertracing/` or `registry.redhat.io/openshift-istio-tech-preview/`
|`tag`
|The tag that the Operator uses to pull the Jaeger images. Any valid container image tag.
|`1.13.1`
|`template`
