...
The istio-operator relies on a custom resource to set up Istio. An example configuration is shown below.
...
|istio
|Kubernetes uses this to identify the resource type
|origin, openshift
|origin
|===
